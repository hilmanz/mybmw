...
class SQLData{
	...
	function openmssql(){
		global $CONFIG;
		$h = $CONFIG['DATABASE']['MSSQL'][0]['HOST'];
		$u = $CONFIG['DATABASE']['MSSQL'][0]['USERNAME'];
		$p = $CONFIG['DATABASE']['MSSQL'][0]['PASSWORD'];
		$d = $CONFIG['DATABASE']['MSSQL'][0]['DATABASE'];
		/*
		$h = 'modist_stg';
		if($ip) $h = '117.54.1.61';
		$u = 'wedhus';
		...
		$d ='modistaging_new';
		*/
		$link = mssql_connect($h, $u, $p)   or die( "<strong>ERROR: Connection to {$h}  failed</strong>" );
		if($link) {
			// echo "success connect to server";
			mssql_select_db($d, $link ) or die( "<strong>ERROR: DB not exists </strong>" );
			
		} 
	}
	
	function fetchmssql($sql=""){
