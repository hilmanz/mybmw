...
class loginHelper {	
	...
	function goLogin(){
		...
		$sql = "
		SELECT *
		FROM {$this->config['DATABASE'][0]['DATABASE']}.sys_users
		...
		$plainpass = $this->decrypt($rs['password']);			
 		if(($plainpass==$password)&&($hash==$rs['password'])){			
				if($rs['status']==1){			
					...
					$logger->log('Able to login BMW CMS');
				...
				$logger->log("Not able to login Cobra CMS ");			 
				return false;
	}
	...
	function add_try_login($rs=false){
		...
		$sql ="UPDATE {$this->config['DATABASE'][0]['DATABASE']}.gm_user SET last_login=now(),try_to_login=try_to_login+1 WHERE id='{$rs['id']}' LIMIT 1";
		$res = $this->apps->query($sql);
		
		$sql = "SELECT try_to_login FROM {$this->config['DATABASE'][0]['DATABASE']}.gm_user WHERE id='{$rs['id']}' LIMIT 1";
		$res = $this->apps->fetch($sql);
		
		if($res){
			if($res['try_to_login']>4) {
				$sql ="UPDATE {$this->config['DATABASE'][0]['DATABASE']}.gm_user SET n_status=9 WHERE id='{$rs['id']}' LIMIT 1";
				$res = $this->apps->query($sql);
			}
		}
	}
	...
	function reset_try_login($rs=false){
		...
		$sql ="UPDATE {$this->config['DATABASE'][0]['DATABASE']}.gm_user SET last_login=now(),try_to_login=0 WHERE id='{$rs['id']}' LIMIT 1";
		$res = $this->apps->query($sql);
	...
	function add_stat_login($user_id){
		...
		$sql ="UPDATE {$this->config['DATABASE'][0]['DATABASE']}.gm_user SET last_login=now(),login_count=login_count+1 WHERE id={$user_id} LIMIT 1";
	...
	function realeaselock(){
			...
			$sql = "
					UPDATE {$this->config['DATABASE'][0]['DATABASE']}.gm_user SET n_status = 1,try_to_login=0
					WHERE username = '{$username}' LIMIT 1				
			";
			// pr($sql);
		...
		if($qData){
			...
			if($socialupdates){
				$qUpdatesocialupdateData = implode(',',$socialupdates);
					...
					$sql = "
						UPDATE {$this->config['DATABASE'][0]['DATABASE']}.gm_user SET {$qUpdatesocialupdateData} 
