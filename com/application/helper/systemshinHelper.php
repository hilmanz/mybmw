...
class systemshinHelper {
	...
	function selectupdatedata(){
		...
		$sql = "select * from {$CONFIG['DATABASE'][0]['DATABASE']}.system where 1 order by id_system ASC ";
		//	pr($sql);exit;
	...
	function editsystemshin($id, $images=false){
		...
		$sql = "UPDATE  {$CONFIG['DATABASE'][0]['DATABASE']}.system set `title`='{$title}',`description`='{$description}',`content`='{$content}',`date`='{$startdate}' where id_system={$id}";
		//pr($sql);exit;
