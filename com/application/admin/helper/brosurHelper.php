...
class brosurHelper {
	...
	function listnews($start=null,$limit=10)
	{
		...
		$sql = "SELECT count(*) total
			FROM {$CONFIG['DATABASE'][0]['DATABASE']}.my_brosur 
			WHERE 1 ";
	...
	//pr($sql);exit;
		...
		$sql = "
			...
			FROM {$CONFIG['DATABASE'][0]['DATABASE']}.my_brosur 
			WHERE 1 
			ORDER BY id_brosur DESC LIMIT {$start},{$limit}
	...
	function addbrosur(){
		...
		$sql = "INSERT INTO {$CONFIG['DATABASE'][0]['DATABASE']}.my_brosur (`title`,`date`) 
	...
	function newsupdate($listeducation=false, $images=false)
	{
		...
		$sql = "UPDATE  {$CONFIG['DATABASE'][0]['DATABASE']}.my_brosur SET files = '{$images}' WHERE id_brosur = {$listeducation}";
		$res = $this->apps->query($sql);
		// pr($sql);exit;
	}		
	function selectupdatedata($id){
		...
		$sql = "select * from {$CONFIG['DATABASE'][0]['DATABASE']}.my_brosur where id_brosur='{$id}' ";
			//pr($sql);exit;
	...
	function editnews($id, $images=false){
		...
		$sql = "UPDATE  {$CONFIG['DATABASE'][0]['DATABASE']}.my_brosur set `title`='{$title}',`description`='{$description}',`content`='{$content}',`date`='{$startdate}' where id_brosur={$id}";
		//pr($sql);exit;
		...
		//pr($images);exit;
			...
			{
			//echo "ss";exit;
			$sql2 = "UPDATE  {$CONFIG['DATABASE'][0]['DATABASE']}.my_brosur SET img = '{$images}' WHERE id_brosur = {$id}";
	...
	function deletenews($id){
		...
		$sql = "DELETE FROM  {$CONFIG['DATABASE'][0]['DATABASE']}.my_brosur where id_brosur={$id}";
		//pr($sql);exit;
