...
class educationHelper {
	...
	function educationupdate($listeducation=false, $images=false)
	{
		global $CONFIG;
		//update
		// pr($images); exit;
		...
		$sql = "UPDATE  {$CONFIG['DATABASE'][0]['DATABASE']}.my_education SET img = '{$images}' WHERE id_education = {$listeducation}";
		$res = $this->apps->query($sql);
		//pr($sql);exit;
	}				
	...
	function editeducation($id, $images=false){
		global $CONFIG;
		...
		if($images!='') 
			{
			//echo "ss";exit;
			$sql2 = "UPDATE  {$CONFIG['DATABASE'][0]['DATABASE']}.my_education SET img = '{$images}' WHERE id_education = {$id}";
