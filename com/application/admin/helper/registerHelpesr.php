...
class registerHelper {
		...
		function brandList()
	{
		global $CONFIG;
		$qProject = "  ";
		if($this->apps->user->type<=100){
			$qProject = " AND id = {$this->apps->user->brand} ";
		}
		
		$brandlist = "
		SELECT * FROM {$CONFIG['DATABASE'][0]['DATABASE']}.tbl_brand_master WHERE 1 {$qProject} ";
			//pr($sql);exit;
		// pr($sql);
	...
	//pr($_POST);
		...
		if(isset($submit)){
			...
			$sql2 = "UPDATE {$CONFIG['DATABASE'][0]['DATABASE']}.my_profile SET `type`='{$role}',`brand`='{$o_brand}',`n_status`='1' where ownerid= {$id} ";		
