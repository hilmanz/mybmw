...
class systemshinHelper {
	...
	//pr($sql);exit;
		...
		$sql = "
			...
			FROM {$CONFIG['DATABASE'][0]['DATABASE']}.system
			WHERE 1 
			ORDER BY id_system ASC LIMIT {$start},{$limit}
	...
	//pr($_POST);exit;
		...
		$sql = "INSERT INTO {$CONFIG['DATABASE'][0]['DATABASE']}.system (`title`, `description`,`content`,`date`) 
	...
	function systemshinupdate($listeducation=false, $images=false)
	{
		global $CONFIG;
		//update
		// pr($images); exit;
		...
		$sql = "UPDATE  {$CONFIG['DATABASE'][0]['DATABASE']}.system SET img = '{$images}' WHERE id_system = {$listeducation}";
		$res = $this->apps->query($sql);
		// pr($sql);exit;
	}		
	function selectupdatedata($id){
		...
		$sql = "select * from {$CONFIG['DATABASE'][0]['DATABASE']}.system where id_system='{$id}' ";
			//pr($sql);exit;
	...
	function editsystemshin($id, $images=false){
		global $CONFIG;
		...
		if($id != '4')
		{
		$sql = "UPDATE  {$CONFIG['DATABASE'][0]['DATABASE']}.system set `title`='{$title}',`description`='{$description}',`date`='{$startdate}' where id_system={$id}";
		...
		//pr($images);exit;
			...
			{
			//echo "ss";exit;
			$sql2 = "UPDATE  {$CONFIG['DATABASE'][0]['DATABASE']}.system SET img = '{$images}' WHERE id_system = {$id}";
	...
	function deletesystemshin($id){
		global $CONFIG;

		$sql = "DELETE FROM  {$CONFIG['DATABASE'][0]['DATABASE']}.system where id_system={$id}";
		//pr($sql);exit;
