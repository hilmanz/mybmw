...
class Application extends SQLData{
	...
	function isUserOnline(){
		global $LOCALE;
		//echo "Ss";exit;
		// pr($this->session->getSession($this->config['SESSION_NAME'],"dashboard"));exit;
		if($this->session->getSession($this->config['SESSION_NAME'],"WEB")) return true;
		return false;
		
	}
	
	function getUserOnline(){
		if($this->session->getSession($this->config['SESSION_NAME'],"WEB")) return $this->session->getSession($this->config['SESSION_NAME'],"WEB");
		else return false;
	...
	function notif($notif=false,$cid=false,$fid=false,$type=false){
		$this->track->setnotificationuser($notif,$cid,$fid,$type);
	}
	
	function apns($fid=false,$notif=false){
		$this->track->apnspasser($fid,$notif);
	}
