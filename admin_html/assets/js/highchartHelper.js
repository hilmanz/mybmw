		 ...
		 // console.log(renderTo);
			...
			chart= new Highcharts.Chart({
				...
				yAxis: {
					...
					labels: {
						 formatter: function () {
							if(renderTo == 'pageview' ||renderTo == 'visit' || renderTo == 'usernewga'  ) return parseInt(this.value);
							if(renderTo == 'avgVisit') return formatTime(parseInt(this.value));
							if(renderTo == 'avgTimeonpage') return  formatTime(parseInt(this.value));
							if(renderTo == 'bounceRate') return  this.value+' %';
						 }
					}
                },
				tooltip: {
					formatter: function() {
						...
						if(renderTo == 'avgVisit') return '<b>'+ this.x +'</b>: '+ formatTime(parseInt(this.y));
						// if(renderTo == 'timepervisit') return '<b>'+ this.x +'</b>: '+ Highcharts.dateFormat('%H:%m:%S', parseInt(this.y)*1000);
						if(renderTo == 'avgTimeonpage') return '<b>'+ this.x +'</b>: '+ formatTime(parseInt(this.y));
